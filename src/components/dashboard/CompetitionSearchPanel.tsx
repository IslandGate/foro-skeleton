...
} from "@/components/dashboard/CompetitionCard";
...
function matchesPrizeType(
  ...
  return selectedOptions.some((option) => {
    ...
    if (normalizedOption === "cash") {
      return (
        normalizedPrize.includes("$") ||
        normalizedPrize.includes("cash") ||
        normalizedPrize.includes("fund")
      );
    }
    if (normalizedOption === "certificate") {
      return (
        normalizedPrize.includes("certificate") ||
        normalizedPrize.includes("qualification")
      );
    }
    if (normalizedOption === "scholarship") {
      return (
        normalizedPrize.includes("scholarship") ||
        normalizedPrize.includes("grant")
      );
    }
...
export default function CompetitionSearchPanel({
  competitions,
}: CompetitionSearchPanelProps) {
  const [query, setQuery] = useState("");
  const [selected, setSelected] = useState<Record<string, string[]>>({});

  const filterCategories = useMemo(
    () => buildFilterCategories(competitions),
    [competitions],
  );
  ...
  const filteredCompetitions = useMemo(() => {
    return competitions.filter((comp) => {
      ...
      if (
        ...
        !subjectFilter.some((item: string) =>
          comp.subjects.some((s) => s === item),
        )
      ) {
        return false;
      }
      ...
      if (
        ...
        !locationFilter.some((item: string) =>
          normalize(comp.location).includes(normalize(item)),
        )
      ) {
        return false;
      }
      ...
      if (
        prizeTypeFilter.length > 0 &&
        !matchesPrizeType(comp.prizeType, prizeTypeFilter)
      ) {
        return false;
      }
      ...
      if (
        groupSizeFilter.length > 0 &&
        !groupSizeFilter.includes(getGroupSize(comp.groupSize))
      ) {
        return false;
      }
    ...
    <div className="flex flex-col gap-6">
      <section className="rounded-xl border border-black/10 bg-white/70 p-4">
        <label
          ...
          className="mb-2 block text-xs font-medium tracking-[2px] text-black/70"
        >
          SEARCH COMPETITIONS
        </label>
        <input
      ...
      <section className="flex flex-col gap-3">
        <p className="text-[18px] font-light leading-[39.8px] tracking-[4px] text-black">
          SEARCH FILTERS
        </p>
        <div className="flex flex-wrap gap-6">
          {filterCategories.map((cat: FilterCategory) => (
            <div
              key={cat.label}
              className="flex min-w-[180px] max-w-[240px] flex-1 flex-col px-[5px]"
            >
              <p className="h-10 text-[10px] font-light leading-[39.8px] tracking-[4px] text-black/60">
                {cat.label}
              </p>
              <div className="h-px w-full bg-black/60" />
              <div
                className="flex flex-col px-[5px] pt-[5px]"
                role="group"
              ...
              >
                {cat.options.map((opt: string) => (
                  ...
                  >
                    <span className="text-[12px] font-medium leading-[34.4px] text-black">
                      {opt}
                    </span>
                    <Checkbox
